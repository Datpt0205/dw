...
| 2 — Runtime/gateway/memory/knowledge/tools | DONE (2026-07-23) | 136 unit + 21 integration tests xanh. LangGraph 1.2.9 sau WorkflowRunnerPort; checkpoint saver tenant-aware (RLS) vào platform.run_checkpoints; demo graph pause tại approval → dựng lại toàn bộ stack (mô phỏng restart) → resume APPROVED/REJECTED đều pass; tool executor 8 bước: replay idempotent (side effect không lặp), conflict detection, deny+audit; model gateway routing (mock deterministic + OpenAI-compatible httpx); Qdrant tenant filter: cross-tenant search chính xác văn bản gốc → 0 kết quả, clearance chặn restricted; memory write policy: auto-write/review/reject với provenance bắt buộc. 6 import-linter contracts KEPT.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 3 — Meeting-to-action slice                | DONE (2026-07-23) | E2E 4/4 pass trên stack thật (API in-process + Postgres + MinIO + mock model): transcript → 14-node graph → pause approval → approve → resume → MockTaskConnector dispatch (external MOCK-ref persisted, idempotent) → memory episodic → audit timeline đầy đủ (run.started/waiting_approval/resumed/tool.executed/run.completed). Reject path không dispatch; member không có approvals.decide bị 403; cross-tenant meeting 404. OpenAPI snapshot + openapi-typescript types + contract tests pass. Web UI: meetings/meeting detail (summary, decisions, actions, run timeline)/approvals inbox/actions/admin session — build pass. Tổng: 145 unit + 25 integration + 2 contract.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 4 — Tender slice                           | DONE (2026-07-23) | E2E 3/3 pass (Postgres+MinIO+Qdrant thật + mock model): documents → 14-node graph → requirements (2 mandatory + 2 weighted) → compliance matrix 8 findings với quote GROUNDED (evidence locator: quote bịa → mandatory fail closed) → deterministic scoring GOLDEN (Thiết bị Việt 87.00 eligible; Vật tư Miền Nam 78.00 nhưng DISQUALIFIED do giao hàng 45>30 ngày) → gate quyết định supplier (draft sai bị override) → approval pause/resume → evaluation pack JSON export vào MinIO (verify object thật) → memory + audit chain. Determinism: 2 case liên tiếp cùng điểm. Cross-tenant 404. Unit: 12 scoring/locator tests. Web: procurement list/detail matrix/suppliers/evaluations. DeepSeek support: json_object fallback + configs/models/deepseek.yaml + env hướng dẫn. Tổng: 159 unit+contract / 28 integration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 5 — Eval/observability/audit/release       | DONE (2026-07-23) | `make eval-smoke` THẬT: 2 dataset (tender 8 case, work_ops 7 case) 15/15 PASS — grader chạy component thật (scoring engine golden 87.00/78.00, evidence locator quote bịa→fail, prompt injection giam trong block untrusted, SearchQuery mang tenant_id bị reject + trusted filter chỉ từ AccessContext, memory no-provenance→reject, ambiguous identity→None, dispatch tool approval=always); đủ 3 security category bắt buộc/dataset. Release manifest: content-addressed sha256 (bỏ git_sha khỏi hash), history immutable, `--check` trong `make ci`; bootstrap đọc manifest.ref → LangGraphWorkflowRunner ghi `release_manifest_ref` vào mọi run — E2E assert pass. Observability: TelemetryPort + NullTelemetry/RecordingTelemetry, OtelTelemetry (span+counter, redaction attributes), Langfuse = OTLP endpoint + Basic auth (không thêm SDK), TelemetryUsageRecorder emit dw_model_tokens_total; wiring qua settings (OTEL_EXPORTER_OTLP_ENDPOINT / LANGFUSE_*), fail-fast khi LANGFUSE_ENABLED thiếu key. Audit viewer /audit (filter + bảng đầy đủ policy/trace/details). CI đầy đủ 10 stage §24.2 (contracts+manifest check, quality, security scan pip-audit+pnpm audit, integration trên compose, eval smoke, container build + compose smoke) + eval-regression.yml (nightly) + release.yml (tag→manifest+artifacts). Gates: ruff/mypy 214 files/176 unit+contract/6 import-linter/declared-deps 12 pkg/web build/`make ci` xanh; E2E tender re-run pass với runner mới. |
| 6 — Hardening & acceptance                 | DONE (2026-07-23) | Hardening: CircuitBreaker (kernel, clock-injected, gắn vào OpenAI-compatible adapter), SSRF guard `ensure_allowed_outbound_url` (chặn private/loopback/metadata; production cấm private), rate-limit middleware per-caller 240/phút (429+Retry-After, health exempt) — 12 unit test. API+UI mới: /knowledge/documents, /memory/items, /integrations (phản chiếu ToolRegistry thật), trang Inbox; member/approver thêm scope knowledge.read+memory.read. Docs: THREAT_MODEL.md (STRIDE→test), 4 runbooks, ADR-015/016/017, README luồng demo. **BƯỚC 4 PASS** (ACCEPTANCE_REPORT.md): blank-DB migrate (29 bảng, 26 RLS FORCE) + seed x2 idempotent + 28/28 integration/E2E trên stack sạch + docker full build (sửa 3 lỗi container thật: tag uv image, tsconfig.base, venv path/DW_REPO_ROOT) → 8/8 container healthy + E2E tender qua API container (golden 87.00, manifest ref, audit chain, export MinIO) + `make ci` xanh.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
